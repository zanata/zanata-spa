(function () {
  ...
  function resizer (SettingsService, $window, $document, $timeout) {
    function link (scope, element, attrs) {
       ...
       * @type {Number}
       */
      scope.height = parseInt(attrs.resizerHeight, 10)
      ...
      function adjustVerticalPanel (resizerPositionX) {
        ...
        var leftPanel = angular.element(document
            .querySelector(attrs.resizerLeft))
        var rightPanel = angular.element(document
            .querySelector(attrs.resizerRight))
        var maximumPanelSize =
            normalisePercentage(attrs.resizerMax, $window.innerHeight)
        var minimumPanelSize = attrs.resizerMin ||
            parseInt(attrs.resizerWidth, 10)
      ...
      function adjustHorizontalPanel (resizerPositionY) {
        ...
        var topPanel = angular.element(document
            .querySelector(attrs.resizerTop))
        var bottomPanel = angular.element(document
            .querySelector(attrs.resizerBottom))
        var maximumPanelSize =
            normalisePercentage(attrs.resizerMax, $window.innerHeight)
        var minimumPanelSize = attrs.resizerMin || scope.actualHeight
      ...
      function normalisePercentage (fraction, whole) {
        if ((/[0-9]*\.?[0-9]+%/).test(fraction)) {
          return Math.round(whole *
            (parseInt(fraction.replace('%', ''), 10) / 100))
        } else {
