...
export const prepareStats = (statistics) => {
  _.forEach(statistics, (statistic) => statistic[getId('needswork')] = statistic['needReview'] || 0);
  // TODO pahuang first is word stats and second is message stats
  const msgStatsStr = _.pick(statistics[1], ['total', 'untranslated', 'rejected', 'needswork', 'translated', 'approved']);
  return _.mapValues(msgStatsStr, (numStr) => parseInt(numStr, 10));
};

export const prepareDocs = (documents) => _.pluck(documents || [], 'name');
